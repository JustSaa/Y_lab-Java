...
public class TransactionServiceImpl implements TransactionService {
    ...
    public void createTransaction(Transaction transaction) {
        if (transaction.getType() == TransactionType.EXPENSE) {
            ...
            double budgetLimit = budgetService.getUserBudget(transaction.getUserId()).getLimit();

            if ((totalExpenses + transaction.getAmount()) > budgetLimit) {
    ...
    @Override
    public List<Transaction> getFilteredTransactions(long userId, TransactionType type, Category category, LocalDate date) {
        if (type != null) {
            return getTransactionsByType(userId, type);
        } else if (category != null) {
            return getTransactionsByCategory(userId, category);
        } else if (date != null) {
            return getTransactionsByDate(userId, date);
        } else {
